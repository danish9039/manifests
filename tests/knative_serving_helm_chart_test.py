...
def fake_commands(temporary):
    ...
    command.write_text(
        ...
        " security=resource['spec']['template']['spec']['containers'][0].get('securityContext',{})\n"
        " assert security.get('allowPrivilegeEscalation') is False and security.get('runAsNonRoot') is True, 'probe violates restricted Pod Security'\n"
        " assert security.get('capabilities',{}).get('drop')==['ALL'] and security.get('seccompProfile',{}).get('type')=='RuntimeDefault', 'probe violates restricted Pod Security'\n"
