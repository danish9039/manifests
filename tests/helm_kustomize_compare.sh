...
if [[ -z "$COMPONENT" ]]; then
    ...
    echo "Components: katib, hub, kserve-models-web-app, kubeflow-trainer"
    exit 1
fi
...
case "$COMPONENT" in
    ...
    "kubeflow-trainer")
        CHART_DIR="$ROOT_DIR/experimental/helm/charts/kubeflow-trainer"
        MANIFESTS_DIR="$ROOT_DIR/applications/trainer"

        declare -A KUSTOMIZE_PATHS=(
            ["crds"]="$MANIFESTS_DIR/overlays"
            ["platform"]="$MANIFESTS_DIR/overlays"
        )
    ...
    *)
        ...
        echo "Supported components: katib, hub, kserve-models-web-app, kubeflow-trainer"
        exit 1
        ;;
esac
...
if [[ "$COMPONENT" == "kserve-models-web-app" ]]; then
    ...
    if [[ "$COMPONENT" == "katib" ]]; then
        helm template katib . \
            ...
            --include-crds \
            --values "$HELM_VALUES_ARG" > "$HELM_OUTPUT"
    elif [[ "$COMPONENT" == "kubeflow-trainer" ]]; then
        helm template kubeflow-trainer . \
            --namespace "$NAMESPACE" \
            --include-crds \
            --values "$HELM_VALUES_ARG" > "$HELM_OUTPUT"
    else
