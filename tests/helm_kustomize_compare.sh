...
if [[ -z "$COMPONENT" ]]; then
    ...
    echo "Components: katib, hub, kserve-models-web-app, kserve"
    exit 1
fi
...
case "$COMPONENT" in
    ...
    "kserve")
        CHART_DIR="$ROOT_DIR/experimental/helm/charts/kserve"
        MANIFESTS_DIR="$ROOT_DIR/applications/kserve/kserve"
    ...
    *)
        ...
        echo "Supported components: katib, hub, kserve-models-web-app, kserve"
        exit 1
        ;;
esac
...
if [[ "$COMPONENT" == "kserve-models-web-app" ]]; then
    ...
    if [[ "$COMPONENT" == "katib" ]]; then
        helm template katib . \
            ...
            --include-crds \
            --values "$HELM_VALUES_ARG" > "$HELM_OUTPUT"
    elif [[ "$COMPONENT" == "kserve" ]]; then
        helm template kserve . \
            --namespace "$NAMESPACE" \
            --include-crds \
            --values "$HELM_VALUES_ARG" > "$HELM_OUTPUT"
    else
