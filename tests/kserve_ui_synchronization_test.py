...
CHART_PATH = Path("applications/kserve/kserve-ui/helm")


class ModelsUiSynchronizationTest(unittest.TestCase):
    def test_replay_is_idempotent_and_commit_leaves_manual_changes_unstaged(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            shutil.copytree(
                root / "applications/kserve/kserve-ui/upstream",
                source / "models-web-app/manifests/kustomize",
            )
            ...
            def synchronize():
                result = subprocess.run(
                    ["bash", "scripts/synchronize-kserve-ui-manifests.sh"],
                    cwd=root,
                    env=environment,
                    capture_output=True,
                    text=True,
