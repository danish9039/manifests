...
def get_resource_key(manifest: Dict, component: str = "katib") -> str:
    ...
    # Include namespace in key for components that render same-name resources in
    # multiple namespaces.
    if component in ["katib", "knative-serving"] and namespace:
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        ...
        print("Components: katib, hub, kserve-models-web-app, knative-serving")
        sys.exit(1)
    ...
    if component not in ["katib", "hub", "kserve-models-web-app", "knative-serving"]:
        print(f"ERROR: Unknown component: {component}")
        print("Supported components: katib, hub, kserve-models-web-app, knative-serving")
        sys.exit(1)
