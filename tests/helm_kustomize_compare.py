...
def get_unique_resource_identity(manifest: Dict) -> str:
    """Generate strict key including namespace to detect true duplicate manifests."""
    kind = manifest.get('kind', 'Unknown')
    name = manifest.get('metadata', {}).get('name', 'unknown')
    namespace = manifest.get('metadata', {}).get('namespace', '')
    scope = namespace if namespace else "_cluster"
...
def build_resource_map(manifests: List[Dict], component: str) -> Tuple[Dict[str, Dict], Dict[str, int]]:
    """Build resource map keyed by kind/name and track duplicate keys."""
    ...
    for manifest in manifests:
        ...
        identity = get_unique_resource_identity(normalized)
        if identity in seen_identity:
            duplicates[identity] = duplicates.get(identity, 1) + 1
        seen_identity[identity] = seen_identity.get(identity, 0) + 1
        if key in resources:
            # keep the last copy for comparison behaviour to match prior script semantics
            pass
...
def compare_manifests(kustomize_file: str, helm_file: str, component: str, scenario: str, namespace: str = "", verbose: bool = False) -> bool:
    ...
    kustomize_resources, kustomize_duplicates = build_resource_map(kustomize_manifests, component)
    helm_resources, helm_duplicates = build_resource_map(helm_manifests, component)
    ...
    if kustomize_duplicates:
        print(f"Duplicate resources in Kustomize output: {len(kustomize_duplicates)}")
        for resource, count in sorted(kustomize_duplicates.items()):
            print(f"  - {resource} ({count} copies)")
        success = False
        differences_found.extend(kustomize_duplicates.keys())

    if helm_duplicates:
        print(f"Duplicate resources in Helm output: {len(helm_duplicates)}")
        for resource, count in sorted(helm_duplicates.items()):
            print(f"  - {resource} ({count} copies)")
        success = False
        differences_found.extend(helm_duplicates.keys())
