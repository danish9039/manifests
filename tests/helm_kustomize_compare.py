    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            ...
            if isinstance(value, str):
                if key == "name" and any(ref_pattern in path for ref_pattern in [
...
def normalize_manifest(manifest: Dict, component: str = "katib", normalize_kustomize_names: bool = True) -> Dict:
    ...
    if normalize_kustomize_names and 'metadata' in normalized and 'name' in normalized['metadata']:
...
def compare_manifests(kustomize_file: str, helm_file: str, component: str, scenario: str, namespace: str = "") -> bool:
    ...
    for manifest in kustomize_manifests:
        if not should_compare_manifest(manifest, component, scenario):
            continue
        normalized = normalize_manifest(manifest, component, normalize_kustomize_names=True)
